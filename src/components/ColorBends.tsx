...
export default function ColorBends({
	...
	useEffect(() => {
		...
		const colorValues = colors.filter(Boolean).slice(0, MAX_COLORS).map(toVec3);
		for (let i = 0; i < MAX_COLORS; i += 1) {
			const color = (material.uniforms.uColors.value as THREE.Vector3[])[i];
			if (i < colorValues.length) color.copy(colorValues[i]);
