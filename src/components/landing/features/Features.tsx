			...
			>
				<div className="flex items-center justify-between px-5 py-4 bg-[#0F0F0F] border-b border-white/5">
					<div className="flex items-center gap-4">
						...
						<span className="text-[11px] text-white/80 font-arimo tracking-[0.15em] uppercase">
							Vince Staples
						</span>
					</div>
				...
				<div className="flex flex-col md:flex-row p-8 md:p-16 md:pb-32 gap-16 items-start">
					<div className="flex-1 w-full space-y-10">
						{lyrics.map((line, index) => {
							...
							if (index === activeIndex) {
								stateClasses =
									"bg-[#1E0D1C] text-white px-6 py-5 rounded-2xl -mx-6";
					...
					<div className="w-full md:w-[320px] flex flex-col items-center flex-shrink-0 sticky top-12">
						...
						<div className="text-center mb-8">
							<h4 className="font-syne font-bold text-sm tracking-[0.15em] text-white uppercase">
								Take Me Home
							</h4>
	...
	const [activities, setActivities] = useState([
		{
			id: 101,
			userId: 1,
			action: "modifie le Refrain",
			time: "il y a 2 min",
		},
		{
			id: 102,
			userId: 5,
			action: "écrit dans le Couplet 1",
			time: "il y a 6 min",
		},
		{
			id: 103,
			userId: 5,
			action: "a importé un fichier WAV",
		...
		const interval = setInterval(() => {
			...
			setActivities((prev) => {
				const updatedFeed = [newActivity, ...prev].map((act, i) => {
					if (i === 1 && act.time === "À l'instant")
						return { ...act, time: "il y a 1 min" };
					return act;
				});
	...
	return (
		<div className="w-full">
			<div className="flex flex-col md:flex-row justify-between items-start mb-12 gap-8">
				...
				<div className="max-w-sm">
					<p className="text-[#888888] font-arimo text-sm mb-4 leading-relaxed">
						Invite tes collaborateurs et travaille sur le même
						projet simultanément. Vois qui écrit quoi, assignez des
						sections, gardez le contrôle de chaque version.
					</p>
					<span className="text-[#D90097] font-arimo text-sm font-bold uppercase tracking-widest cursor-default">
						Collaboration →
					</span>
						...
						<div className="space-y-10 min-h-[400px]">
							{activities.map((activity, index) => {
								...
								const isTypingAction =
									activity.action.includes("modifie") ||
									activity.action.includes("écrit");
								...
								return (
									<div
										...
										className={`flex items-start gap-5 transition-all duration-700 ease-out ${
											isNew
												? "animate-feature-slide-in text-white"
												: "text-white/80"
										}`}
									>
										<div className="relative flex-shrink-0 w-14 h-14">
											...
											<div
												className={`absolute inset-0 rounded-full flex items-center justify-center font-bold text-white text-lg shadow-inner ${user?.color} ${isNew && isTypingAction ? "animate-avatar-show" : ""}`}
											>
												{user?.name.charAt(0)}
											</div>
						...
						<div className="p-6 space-y-3 max-h-[420px] overflow-y-auto custom-scrollbar">
							{filteredUsers.map((user) => (
								...
								>
									<div className="flex items-center gap-5">
										<div
											className={`w-12 h-12 rounded-full flex items-center justify-center font-bold text-white text-md ${user.color}`}
										>
											{user.name.charAt(0)}
										</div>
										<span
											className={`font-arimo text-[17px] transition-colors ${user.selected ? "text-white" : "text-white/50 group-hover:text-white/80"}`}
										>
											{user.name}
										</span>
									</div>
							...
							{filteredUsers.length === 0 && (
								<div className="p-8 text-center">
									<p className="text-white/20 font-arimo text-sm">
			...
			>
				...
				<div className="flex-1 w-full flex flex-col relative z-10 bg-[#0F0F0F]">
					<div className="flex items-center justify-between px-5 py-4 border-b border-white/5 bg-[#0A0A0A]">
						...
						<span className="text-[11px] text-white/50 font-arimo tracking-[0.15em] uppercase">
					...
					<div className="flex flex-col md:flex-row flex-1">
						...
						<div className="flex-1 p-8 md:p-16 border-r border-white/5 font-arimo text-[20px] md:text-[24px] leading-[2.2] tracking-wide relative">
							...
							<div
								className={`absolute bottom-8 left-16 flex items-center gap-3 text-white/30 text-sm font-syne transition-opacity duration-300 ${step === 0 ? "opacity-100" : "opacity-0"}`}
							>
						...
						<div className="w-full md:w-[400px] bg-[#0A0A0A] p-6 md:p-10 flex flex-col relative overflow-hidden">
							<div className="flex items-center gap-4 mb-10 relative z-10">
								...
								<div>
									<h3 className="font-syne text-white text-lg font-bold tracking-wide">
										Dictionnaire
									</h3>
							...
							>
								<div className="bg-[#111111] border border-white/5 rounded-2xl p-6 mb-8 shadow-2xl relative overflow-hidden group">
									...
									<p className="font-syne text-3xl font-bold text-white mb-2 tracking-tight">
										« nuit »
									</p>
								...
								<div className="space-y-3">
									{[
										...
										>
											<div className="flex items-center gap-4">
												<span
													className={`text-[17px] ${step >= 3 && idx === 0 ? "font-bold text-[#D90097]" : ""}`}
												>
													{item.word}
												</span>
											</div>
				...
				<div className="absolute inset-0 z-0 pointer-events-none overflow-hidden">
					<div
						className={`absolute top-1/2 right-0 -translate-y-1/2 w-[500px] h-[500px] bg-[#D90097]/10 blur-[100px] rounded-full mix-blend-screen transition-opacity duration-1000 ${
							step >= 2 && step <= 4
