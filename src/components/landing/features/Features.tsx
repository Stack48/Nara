        ...
        <div className="w-full">
            ...
            <div className="relative w-full min-h-[500px] md:min-h-[600px] bg-[#0A0A0A] rounded-3xl overflow-hidden border border-white/10 shadow-2xl flex flex-col z-10">
                ...
                <div className="flex items-center justify-between px-5 py-4 border-b border-white/5 bg-[#0F0F0F]/80 backdrop-blur-md relative z-20">
                    ...
                    <span className="text-[10px] md:text-xs font-arimo text-white/50 tracking-wider uppercase truncate max-w-[150px] md:max-w-none">Live Prompter - Mode Studio</span>
                    <div className="flex gap-2 items-center">
                        <div className="flex items-center gap-2 px-2 py-1 rounded bg-white/5 border border-white/10">
                            <div className={`w-2 h-2 rounded-full ${isPlaying ? 'bg-[#27C93F] animate-pulse' : 'bg-[#FFBD2E]'}`}></div>
                            <span className="text-[10px] text-white/70 uppercase tracking-wider font-bold">{isPlaying ? 'Live' : 'Standby'}</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex flex-col md:flex-row p-8 md:p-16 md:pb-32 gap-16 items-start relative z-10 bg-transparent flex-1">
                    <div className="flex-1 w-full space-y-10">
                        {lyrics.map((line, index) => {
                            ...
                            if (index === activeIndex) {
                                stateClasses = "bg-[#1E0D1C] text-white px-6 py-5 rounded-2xl -mx-6 shadow-xl border border-[#D90097]/20";
                    ...
                    <div className="w-full md:w-[320px] flex flex-col items-center flex-shrink-0 md:sticky md:top-12 mt-8 md:mt-0 bg-[#0F0F0F]/50 backdrop-blur-md p-6 rounded-3xl border border-white/5 shadow-xl">
                        ...
                        <div className="text-center mb-8">
                            <h4 className="font-unbounded font-bold text-sm tracking-[0.15em] text-white uppercase">Take Me Home</h4>
...
const Collaboration = () => {
    ...
    const momoPos =
        step === 0 ? { top: '80%', left: '90%', opacity: 0 } :
            step >= 1 ? { top: '15%', left: '75%', opacity: 1 } : { top: '80%', left: '90%', opacity: 0 };

    const sarahPos =
        step <= 3 ? { top: '90%', left: '10%', opacity: 0 } :
            step >= 4 ? { top: '45%', left: '80%', opacity: 1 } : { top: '90%', left: '10%', opacity: 0 };
        ...
        <div className="w-full">
            ...
            <div className="relative w-full min-h-[500px] md:min-h-[600px] bg-[#0A0A0A] rounded-3xl overflow-hidden border border-white/10 shadow-2xl flex flex-col z-10">
                ...
                <div className="flex items-center justify-between px-5 py-4 border-b border-white/5 bg-[#0F0F0F]/80 backdrop-blur-md relative z-20">
                    ...
                    <span className="text-[10px] md:text-xs font-arimo text-white/50 tracking-wider uppercase truncate max-w-[150px] md:max-w-none">Couplet 2 - Sans Titre</span>
                    <div className="flex gap-2 items-center">
                        <div className="flex -space-x-2">
                            <div className="w-6 h-6 md:w-7 md:h-7 rounded-full bg-[#D90097] border-2 border-[#0F0F0F] flex items-center justify-center text-[9px] md:text-[10px] font-bold text-white z-20">M</div>
                            <div className="w-6 h-6 md:w-7 md:h-7 rounded-full bg-[#27C93F] border-2 border-[#0F0F0F] flex items-center justify-center text-[9px] md:text-[10px] font-bold text-white z-10">S</div>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex-1 p-6 sm:p-8 md:p-14 lg:p-20 font-arimo text-[19px] sm:text-2xl md:text-3xl leading-[2] md:leading-[2.2] text-white/80 relative overflow-hidden bg-transparent z-10">
                    ...
                    <div
                        ...
                        style={{ top: momoPos.top, left: momoPos.left, opacity: momoPos.opacity, transform: 'translate(-5px, -5px)' }}
                    >
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" className="drop-shadow-xl scale-75 md:scale-100 origin-top-left">
                            <path d="M5.5 3.5L19.5 10.5L12 12.5L9.5 20L5.5 3.5Z" fill="#D90097" stroke="white" strokeWidth="1.5" strokeLinejoin="round" />
                        </svg>
                    ...
                    <div
                        className="absolute z-50 transition-all duration-1000 ease-[cubic-bezier(0.34,1.56,0.64,1)] pointer-events-none"
                        style={{ top: sarahPos.top, left: sarahPos.left, opacity: sarahPos.opacity, transform: 'translate(-5px, -5px)' }}
                    >
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" className="drop-shadow-xl scale-75 md:scale-100 origin-top-left">
                            <path d="M5.5 3.5L19.5 10.5L12 12.5L9.5 20L5.5 3.5Z" fill="#27C93F" stroke="white" strokeWidth="1.5" strokeLinejoin="round" />
                        </svg>
                    ...
                    <p className="whitespace-pre-wrap relative z-10 w-full sm:w-11/12 md:w-3/4">
                        On cherchait la lumière dans la <span className="relative inline-block">
                            {step < 2 ? "ville" : step === 2 ? <span className="bg-[#D90097]/40 text-white px-1 -mx-1 rounded">ville</span> : <span className="text-[#D90097] font-bold">nuit</span>}
                            {step >= 3 && step < 4 && <span className="absolute -right-1 md:-right-2 top-1/2 -translate-y-1/2 w-0.5 h-6 md:h-8 bg-[#D90097] animate-pulse"></span>}
                        </span>
                    </p>

                    <div className="mt-2 md:mt-4 relative w-full sm:w-11/12 md:w-3/4 z-10">
                        <p className="whitespace-pre-wrap">
                            Ils ont des choses à dire, mais n'ont rien à <span className="relative inline-block">
                                {step < 5 ? "prouver" : <span className="bg-[#27C93F]/40 text-white px-1 -mx-1 rounded">prouver</span>}
                            </span>
                        </p>

                        {/* Sarah Comment Box */}
                        <div className={`absolute top-full left-0 mt-4 sm:mt-6 bg-[#111]/95 backdrop-blur-xl border border-[#27C93F]/30 p-4 sm:p-5 rounded-2xl shadow-2xl w-[90vw] sm:w-[320px] max-w-[340px] z-40 transition-all duration-500 origin-top-left ${step >= 5 ? 'opacity-100 scale-100 visible' : 'opacity-0 scale-95 invisible'}`}>
                            <div className="flex items-center gap-3 mb-2 sm:mb-3">
                                <div className="w-6 h-6 rounded-full bg-[#27C93F] flex items-center justify-center text-[10px] font-bold text-white shadow-inner">S</div>
                                <span className="text-xs sm:text-sm text-white/90 font-bold">Sarah</span>
                            </div>
                            <p className="text-[13px] sm:text-sm text-white/80 font-arimo leading-relaxed">On devrait plutôt dire <span className="text-[#27C93F] font-bold">raconter</span> ici non ? Ça rimerait mieux avec le couplet précédent.</p>
                        </div>
                    </div>

                    <p className="text-white/30 mt-2 md:mt-4 blur-[1px] select-none">Je continue d'écrire pour m'en sortir</p>
                    <p className="text-white/20 mt-2 md:mt-4 blur-[2px] hidden sm:block select-none">Le temps passe vite quand on doit fuir</p>
            ...
            <div className="relative w-full min-h-[500px] md:min-h-[600px] bg-[#0A0A0A] rounded-3xl overflow-hidden border border-white/10 shadow-2xl flex flex-col z-10">
                ...
                <div className="flex-1 w-full flex flex-col relative z-10 bg-transparent">
                    <div className="flex items-center justify-between px-5 py-4 border-b border-white/5 bg-[#0F0F0F]/80 backdrop-blur-md relative z-20">
                        ...
                        <span className="text-[10px] md:text-xs font-arimo text-white/50 tracking-wider uppercase truncate max-w-[150px] md:max-w-none">Couplet 1 - Nara Studio</span>
                        <div className="flex gap-2 items-center">
                            <div className="w-6 h-6 md:w-7 md:h-7 rounded-full bg-[#D90097] border-2 border-[#0F0F0F] flex items-center justify-center text-[9px] md:text-[10px] font-bold text-white z-20">N</div>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col md:flex-row flex-1 relative z-10">
                        ...
                        <div className="flex-1 p-6 sm:p-8 md:p-16 border-b md:border-b-0 border-white/5 md:border-r font-arimo text-[20px] md:text-[24px] leading-[2.2] tracking-wide relative bg-transparent">
                            ...
                            <div className={`absolute bottom-4 left-6 md:bottom-8 md:left-16 flex items-center gap-3 text-white/30 text-sm font-unbounded transition-opacity duration-300 ${step === 0 ? "opacity-100" : "opacity-0"}`}>
                        ...
                        <div className="w-full md:w-[400px] bg-[#0A0A0A]/40 backdrop-blur-md p-6 md:p-10 flex flex-col relative overflow-hidden">
                            <div className="flex items-center gap-4 mb-10 relative z-10">
                                ...
                                <div>
                                    <h3 className="font-unbounded text-white text-lg font-bold tracking-wide">Dictionnaire</h3>
                            ...
                            <div className={`transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] relative z-10 ${step >= 2 ? "opacity-100 translate-x-0" : "opacity-0 translate-x-8 pointer-events-none"}`}>
                                ...
                                <div className="space-y-3">
                                    {[
                                        ...
                                        <div key={idx} className={`p-4 rounded-xl font-arimo transition-all flex items-center justify-between border ${step >= 3 && idx === 0
                                            ...
                                            <div className="flex items-center gap-4">
                                                <span className={`text-[17px] ${step >= 3 && idx === 0 ? "font-bold text-white" : ""}`}>{item.word}</span>
                                            </div>
                                            <div className="flex items-center gap-4">
                            ...
                            <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-center w-full transition-all duration-500 delay-100 ${step < 2 ? "opacity-100" : "opacity-0 pointer-events-none"}`}>
                                ...
                                <p className="text-white/40 text-sm font-arimo tracking-wide">Analyse en temps réel...</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden">
                    <div className={`absolute top-1/2 right-0 -translate-y-1/2 w-[500px] h-[500px] bg-[#D90097]/15 blur-[120px] rounded-full mix-blend-screen transition-opacity duration-1000 ${step >= 2 && step <= 4 ? "opacity-100 scale-110" : "opacity-30 scale-100"
                        }`}></div>
                </div>
            </div>
        </div>
