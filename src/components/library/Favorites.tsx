...
export const Favorites = () => {
    ...
    const favoritesSongs = allSongs.filter((song) => song.isFavorite && !song.isDeleted);

    const allProjects = useProjects();
    const favoritesProjects = allProjects.filter((project) => project.isFavorite && !project.isDeleted);
    ...
    const filteredSongs = favoritesSongs.filter((song) =>
        song.title.toLowerCase().includes(searchQuery.toLowerCase()) ||
        (song.projectName && song.projectName.toLowerCase().includes(searchQuery.toLowerCase()))
    );

    const filteredProjects = favoritesProjects.filter((project) =>
        ...
        <div className="w-full font-arimo text-white pb-10">
            ...
            <div className="flex items-center justify-between gap-3 mb-8">
                <div className="text-sm font-semibold text-neutral-400">
                    {totalFavoritesCount} favorited {totalFavoritesCount === 1 ? "item" : "items"}
                </div>
