...
export const focusTextareaTopPaddingPx = 56;
export const focusTextareaLeftPaddingPx = 64;
...
function getVisibleSectionLines(section: TipTapLyricSection): TipTapLyricLine[] {
	return section.lines;
}

function getActualLineMappings(
	text: string,
...
export default function FocusLyricsDocument({
	...
	handleToggleSectionOption: (sectionId: string, key: SectionOptionKey) => void;
	...
	useEffect(() => {
		if (openAddMenuSectionId === null && openOptionsMenuSectionId === null) {
			return;
		}
		...
		function handlePointerDown(event: globalThis.PointerEvent): void {
			...
			const menuContainer = window.document.querySelector(".section-menu-container");
			const handlesContainer = window.document.querySelector(".section-handles-container");
	...
	}, [openAddMenuSectionId, openOptionsMenuSectionId, setOpenAddMenuSectionId, setOpenOptionsMenuSectionId]);

	const fontSize = typeof lineStyle?.fontSize === "number"
		? lineStyle.fontSize
		: typeof lineStyle?.fontSize === "string"
			? Number.parseInt(lineStyle.fontSize, 10) || 16
			: 16;

	const lineHeightMultiplier = typeof lineStyle?.lineHeight === "number"
	...
	const mappings = useMemo(
		(): FocusLineMapping[] => getActualLineMappings(text, document.sections),
	...
	const focusSectionRanges = useMemo(() => {
		return focusSectionMarkers.map((marker, index) => {
			...
			const endLine = index < focusSectionMarkers.length - 1
	...
	const textareaHeight = Math.max(
		240,
		lineCount * dynamicLineHeightPx + focusTextareaTopPaddingPx + 12,
	);
	...
	return (
		...
		>
							...
							<span
								...
								style={{
									...
									transition: draggedSectionId
										? "transform 0.25s cubic-bezier(0.16, 1, 0.3, 1)"
										: "none",
									opacity: draggedSectionId === marker.id ? 0 : 1,
								}}
							>
								{marker.label}
							</span>
			...
			>
				{focusSectionRanges.map(({ marker, startLine, heightLines }) => {
					const sectionTop = focusTextareaTopPaddingPx + startLine * dynamicLineHeightPx;
						...
						>
							...
							>
								...
								<button
									...
									aria-expanded={openOptionsMenuSectionId === marker.id}
									draggable
									onClick={(): void => onToggleOptionsMenu(marker.id)}
									onDragStart={(event: DragEvent<HTMLButtonElement>): void => {
										const height = heightLines * dynamicLineHeightPx;
										setDraggedSectionId(marker.id);
										setDraggedHeight(height + 20); // mb-5 gap (20px)
										setDragOverSectionId(null);
										event.dataTransfer.effectAllowed = "move";
										event.dataTransfer.setData("application/x-nara-section-id", marker.id);

										// Create custom drag image preview
										const linesText = text.split("\n");
										const sectionLines = linesText.slice(startLine, startLine + heightLines);
										const dragImg = createFocusSectionDragImage(
											marker.label,
											sectionLines,
							...
							{/* Add Menu */}
							{openAddMenuSectionId === marker.id && (
								<div className="section-menu-container absolute left-0 top-0 z-30">
									<SectionAddMenu
										onAddSection={(kind: SectionKind): void => {
											handleAddSection(marker.id, kind);
										}}
										onAddLine={(): void => {
							...
							{openOptionsMenuSectionId === marker.id && (
								<div className="section-menu-container absolute left-0 top-0 z-30">
									<SectionOptionsMenu
										options={getSectionOptions(marker.id)}
										onToggleOption={(key: SectionOptionKey): void => handleToggleSectionOption(marker.id, key)}
										onCreateAlternative={(): void => handleCreateSectionAlternative(marker.id)}
										onDuplicate={(): void => handleDuplicateSection(marker.id)}
										onDelete={(): void => handleDeleteSection(marker.id)}
			...
			{draggedSectionId && (
				...
				>
					{focusSectionRanges.map(({ marker, startLine, heightLines }) => {
						...
						const lines = text.split("\n");
						const sectionLines = lines.slice(startLine, startLine + heightLines);
						...
						return (
							...
							>
								{sectionLines.map((lineText, idx) => {
									const lineTop = idx * dynamicLineHeightPx;
									return (
										<div
											key={idx}
